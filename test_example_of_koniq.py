...
def test(model,  test_loader, epoch, device, all_test_loss):
    ...
    if dataset=='koniq':
        ind = np.where(tg < 2)[0]
        print('Low Quality Pearson of KonIQ:', pd.Series((op[ind])).corr((pd.Series(tg[ind])), method="pearson"))
        print('Low Quality Spearman of KonIQ::', pd.Series((op[ind])).corr((pd.Series(tg[ind])), method="spearman"))
...
def main():
    ...
    model.load_state_dict(torch.load('IEIT_koniq10k.pt'))
...
################################################################
    ...
    all_data = sio.loadmat('E:\Database\IQA Database\## KonIQ-10k Image Database\Koniq_244_deblur_and_brt.mat')
    X2 = all_data['X']
    Xtest2 = all_data['Xtest']
    del all_data

    all_data = np.load('koniq_prob_compare.npz')
    ...
    all_data = np.load('E:\Database\SPAQ\spaq_244_brt_dblur.npz')
    X2 = all_data['X']
    Xtest2 = all_data['Xtest']
    del all_data

    all_data = np.load('spaq_prob_compare.npz')
    op_train = all_data['op_train']
    op_train2 = all_data['op_train2']
    op_test = all_data['op_test']
    op_test2 = all_data['op_test2']
    ...
    X2[ind_train] = X[ind_train]
    Xtest2[ind_test] = Xtest[ind_test]
    test_dataset = Mydataset(np.concatenate((X, Xtest),axis=0), np.concatenate((X2, Xtest2),axis=0), np.concatenate((Y, Ytest),axis=0))
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers_test,pin_memory=True)
...
# # #####################################
    ...
    all_data = sio.loadmat('E:\Database\RBID\\rbid_244_brt_debulr.mat')
    X2 = all_data['X']
    Xtest2 = all_data['Xtest']
    del all_data

    all_data = np.load('rbid_prob_compare.npz')
    op_train = all_data['op_train']
    op_train2 = all_data['op_train2']
    op_test = all_data['op_test']
    op_test2 = all_data['op_test2']

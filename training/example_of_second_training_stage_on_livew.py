...
class Mydataset2(Dataset):
    def __init__(self, imgs, imgs2, labels, imgs_poor, imgs2_poor, labels_poor):
        self.imgs = imgs
        self.imgs2 = imgs2
        self.labels = torch.FloatTensor(labels)
        self.imgs_poor = imgs_poor
        self.imgs2_poor = imgs2_poor
        self.labels_poor= torch.FloatTensor(labels_poor)

    def __getitem__(self, index):
        return torch.from_numpy(self.imgs[index]), torch.from_numpy(self.imgs2[index]),  self.labels[index],torch.from_numpy(self.imgs_poor[index%((self.imgs_poor).shape[0])]), torch.from_numpy(self.imgs2_poor[index%((self.imgs_poor).shape[0])]), self.labels_poor[index%((self.imgs_poor).shape[0])]
...
def train(model,  train_loader, optimizer,  epoch, device, all_train_loss):
    ...
    tg=[]
    tg_poor = []
    for batch_idx, (data,data2, target,data_poor,data2_poor, target_poor) in enumerate(train_loader):
        data,data2, target,data_poor,data2_poor, target_poor = data.to(device),data2.to(device),  target.to(device), data_poor.to(device),data2_poor.to(device),  target_poor.to(device)
        ...
        data_poor = data_poor[:, :, rd_ps[0]:rd_ps[0] + 224, rd_ps[1]:rd_ps[1] + 224]
        data2_poor = data2_poor[:, :, rd_ps[0]:rd_ps[0] + 224, rd_ps[1]:rd_ps[1] + 224]
        
        if rd_ps[1] < 10:
            data = torch.flip(data, dims=[3])
            data2 = torch.flip(data2, dims=[3])
            data_poor = torch.flip(data_poor, dims=[3])
            data2_poor = torch.flip(data2_poor, dims=[3])
        ...
        optimizer.zero_grad()
        output, output2 = model(data, data2,data_poor, data2_poor)

        loss1 = F.l1_loss(output, target)
        ind=torch.nonzero(target<2)[:,0]
        loss2 = F.l1_loss(output2, target_poor)#+5*F.l1_loss(output2[ind],target[ind])
        loss=loss1+loss2
        ...
        tg_poor = np.concatenate((tg_poor, target_poor[:, 0].cpu().numpy()))
    ...
    print( 'Train ALL Pearson:', pd.Series((op[::1])).corr((pd.Series(tg[::1])), method="pearson"),' ALL Pearson2:', pd.Series((op2[::1])).corr((pd.Series(tg_poor[::1])), method="pearson"))
    print( 'Train  ALL Spearman:', pd.Series((op[::1])).corr((pd.Series(tg[::1])), method="spearman"),' ALL Spearman2:', pd.Series((op2[::1])).corr((pd.Series(tg_poor[::1])), method="spearman"))
    ind = np.where(tg < 2)[ 0]
    print('Low Quality Pearson:', pd.Series((op[ind])).corr((pd.Series(tg[ind])), method="pearson"),'Low Quality Pearson2:', pd.Series((op2[ind])).corr((pd.Series(tg_poor[ind])), method="pearson"))
    print('Low Quality Spearman:', pd.Series((op[ind])).corr((pd.Series(tg[ind])), method="spearman"),'Low Quality Spearman2:', pd.Series((op2[ind])).corr((pd.Series(tg_poor[ind])), method="spearman"))
...
def test(model,  test_loader, epoch, device, all_test_loss):
    ...
    tg = []
    with torch.no_grad():
        for batch_idx, (data, data2, target,data_poor, data2_poor, target_poor ) in enumerate(test_loader):
            data, data2, target, data_poor, data2_poor, target_poor = data.to(device), data2.to(device), target.to(device), data_poor.to(device), data2_poor.to(device), target_poor.to(device)

            data = data[:, :,10:10+ 224, 10:10+ 224]
            data2 = data2[:, :, 10:10 + 224, 10:10 + 224]
            data_poor = data_poor[:, :, 10:10 + 224, 10:10 + 224]
            data2_poor = data2_poor[:, :, 10:10 + 224, 10:10 + 224]
            ...
            output1, output2 = model(data, data2, data_poor, data2_poor)
            loss1 = F.l1_loss(output1, target)
            ind = torch.nonzero(target < 2)[:, 0]
            loss2 = F.l1_loss(output2, target_poor) #+ 5 * F.l1_loss(output2[ind], target[ind])
            loss = loss1 + loss2
...
class MyModel3(nn.Module):
    ...
    def forward(self, data,data2,data_poor,data2_poor):
        with torch.no_grad():
            opf1 = self.model1(data)
            opf2_1 = self.model2(data)
            opf2_2 = self.model2(data2)
        opf_poor1 = self.model2(data_poor)
        opf_poor2 = self.model2(data2_poor)

        op1 = self.model3(torch.cat((opf1,opf2_1.detach(),opf2_2.detach()), 1))
        op2 = self.model4(torch.cat((opf_poor1, opf_poor2), 1))
...
def main():
    ...
    # train_loader = DataLoaderX(train_dataset, batch_size=64 * 4, shuffle=True, num_workers=0, pin_memory=True)
    # test_loader = DataLoaderX(test_dataset, batch_size=64 * 4, shuffle=True, num_workers=0, pin_memory=True)
    # _, op_train = test_prob(model2, train_loader, -1, device, [])
    # _, op_test = test_prob(model2, test_loader, -1, device, [])
    ...
    del all_data
    # #
    # train_dataset = Mydataset(X2, Y)
    # test_dataset = Mydataset(Xtest2, Ytest)
    # train_loader = DataLoaderX(train_dataset, batch_size=64 * 4, shuffle=True, num_workers=0, pin_memory=True)
    # test_loader = DataLoaderX(test_dataset, batch_size=64 * 4, shuffle=True, num_workers=0, pin_memory=True)
    # _, op_train2 = test_prob(model2, train_loader, -1, device, [])
    # _, op_test2 = test_prob(model2, test_loader, -1, device, [])
    # np.savez('livew_prob_compare.npz',op_train=op_train,op_test=op_test,op_train2=op_train2,op_test2=op_test2)
    ...
    for i in range(0,10):
        ...
        model = BalancedDataParallel(42 // 1, model, dim=0).to(device)

        model.load_state_dict(torch.load('livew244_deit_enhance_10split'+str(i)+'.pt'))
        ###################################################################


        train_dataset = Mydataset2(X,X2, Y,X_poor ,X2_poor , Y_poor )
        test_dataset = Mydataset2(Xtest, Xtest2,Ytest,Xtest, Xtest2,Ytest )
        ...
        all_train_loss = []
        all_test_loss = []
        all_test_loss, test_loss, _ ,_,_= test(model,test_loader, -1, device, all_test_loss)
        ct = 0
        lr = 0.001
        max_plsp = -2
        ...
        for epoch in range(epochs):
            ...
            all_test_loss, plsp, _ ,lowpl,_ = test(model,test_loader, epoch, device, all_test_loss)
            print("time:", time.time() - start)
            if epoch == 20:
                lr = 0.003

            if max_plsp < plsp+lowpl:
                save_nm ='livew244_deit_enhance_10split_ftpoor'+str(i)+'.pt'
                max_plsp = plsp+lowpl
